<script>
    import { page } from "$app/stores";
</script>

<header>
    <nav class="bg-base-100 p-4 shadow-lg">
        <div class="container mx-auto flex items-center justify-between">
            <a href="/" class="text-2xl font-extrabold text-accent">BITX</a>
            <div class="flex items-center space-x-4">
                <a href="/coins" class="text-accent hover:text-neutral">Coins</a
                >
                <a href="/top-pick" class=" text-accent hover:text-neutral"
                    >Buy</a
                >
                <a href="/learn" class="text-accent hover:text-neutral">Learn</a
                >
                <a href="/market-cap" class="text-accent hover:text-neutral"
                    >Market Cap</a
                >
                <a href="/api" class="text-accent hover:text-neutral">API</a>
                <a href="/cart" class="text-accent hover:text-neutral">Cart</a>
                <!-- <a href="/login" class="btn btn-accent text-base-100">Login</a>
		<a href="/signup" class="btn btn-neutral text-base-100">Sign Up</a> -->
            </div>
        </div>
    </nav>
</header>

<style>
    .container {
        max-width: 1280px;
        padding: 0 1rem;
    }
</style>
