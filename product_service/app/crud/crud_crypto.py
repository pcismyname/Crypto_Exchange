...
def update_cryptocurrency(db: Session, crypto_id: int, cryptocurrency_data: CryptocurrencyUpdate) -> Cryptocurrency:
...
def delete_cryptocurrency(db: Session, crypto_id: int):
    db_cryptocurrency = db.query(Cryptocurrency).filter(Cryptocurrency.crypto_id == crypto_id).first()
    if db_cryptocurrency:
